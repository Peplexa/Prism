"""
Management command to load PoliticalBias AllSides dataset.
"""
...
class Command(BaseCommand):
    help = "Load PoliticalBias AllSides dataset into database"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=None,
            help="Limit number of articles to load",
        )
        parser.add_argument(
            "--extract-facts",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        source, created = DataSource.objects.get_or_create(
            ...
            defaults={
                ...
                "loader_class": "apps.datasets.loaders.PoliticalBiasLoader",
            },
        )
        ...
        self.stdout.write("Loading PoliticalBias AllSides dataset (this may download data)...")

        docs_created = 0
        docs_skipped = 0
        facts_created = 0

        for doc_data in tqdm(loader.load(split=split, limit=limit), desc="Loading articles"):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Loaded {docs_created} articles, skipped {docs_skipped} existing"
            )
        )
        if extract_facts:
            self.stdout.write(
